...
# Calculate class weights for both targets
adhd_weight = dict(enumerate(np.bincount(y_full['ADHD_Outcome']) / len(y_full['ADHD_Outcome'])))
sex_weight = dict(enumerate(np.bincount(y_full['Sex_F']) / len(y_full['Sex_F'])))
adhd_class_weights = {k: 1/v for k, v in adhd_weight.items()}
sex_class_weights = {k: 1/v for k, v in sex_weight.items()}
...
lgb_params_adhd = {
    ...
    'n_estimators': 200,
    'verbose': -1,
    'class_weight': adhd_class_weights  # Add class weights
}
...
lgb_params_sex = {
    ...
    'n_estimators': 200,
    'verbose': -1,
    'class_weight': sex_class_weights  # Add class weights
}
...
for fold, (train_idx, val_idx) in enumerate(skf.split(X, y_full['ADHD_Outcome'])):
    ...
    X_train_adhd_resampled, y_train_adhd_resampled = smote.fit_resample(X_train_processed, y_train_adhd)
    X_train_sex_resampled, y_train_sex_resampled = smote.fit_resample(X_train_processed, y_train_sex)
    
    # Give extra weight to female ADHD cases
    sample_weights_adhd = np.ones(len(y_train_adhd_resampled))
    female_adhd_mask = (y_train_adhd_resampled == 1) & (y_train_sex_resampled == 1)
    sample_weights_adhd[female_adhd_mask] = 2
    
    # Train ADHD model
    model_adhd = lgb.LGBMClassifier(**lgb_params_adhd)
    model_adhd.fit(
        X_train_adhd_resampled, y_train_adhd_resampled,
        sample_weight=sample_weights_adhd,
    ...
    model_sex.fit(
        X_train_sex_resampled, y_train_sex_resampled,
    ...
    # Print fold scores
    print(f"Fold {fold+1} ADHD F1:", f1_score(y_val_adhd, (oof_preds_adhd[val_idx] > 0.5).astype(int)))
    print(f"Fold {fold+1} Sex F1:", f1_score(y_val_sex, (oof_preds_sex[val_idx] > 0.5).astype(int)))
...
# Calculate and print final validation scores
...
#after 5 folds
# Fold 5 ADHD F1: 0.874251497005988
# Fold 5 Sex F1: 0.40350877192982454

# Final Validation F1 Scores:
# ADHD: 0.8451730418943534
# Sex: 0.4193061840120664